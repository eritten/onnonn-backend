    ...
    <div className="flex min-h-screen bg-gradient-to-br from-brand-950 via-brand-900 to-brand-950">
      <aside className={`flex min-h-screen flex-col border-r border-brand-800 bg-[#0F172A] transition-all ${sidebarCollapsed ? "w-24" : "w-72"} p-4`}>
        <div className="flex items-center justify-between">
          ...
          <button className="btn-secondary px-2 py-2" onClick={toggleSidebar}>{sidebarCollapsed ? <ChevronRight size={18} /> : <ChevronLeft size={18} />}</button>
        </div>
        ...
        <div className="mt-auto pt-10">
          <div className="panel p-4">
            ...
            {!sidebarCollapsed && (
              <button className="btn-secondary mt-4 w-full" onClick={() => auth.logout().then(() => navigate("/login"))}>
      ...
      <div className="relative flex min-h-screen flex-1 flex-col">
        <header className="flex items-center gap-4 border-b border-brand-800 bg-brand-950/70 px-8 py-4 backdrop-blur">
          ...
          <button className="btn-secondary" onClick={toggleNotifications}>
            <Bell size={18} />
          ...
          <button className="btn-primary" onClick={() => navigate("/app/meetings?new=instant")}><Home size={16} className="mr-2" />New Meeting</button>
          <button className="btn-secondary" onClick={() => navigate("/app/meetings?new=scheduled")}><CalendarPlus2 size={16} className="mr-2" />Schedule Meeting</button>
        </header>
        ...
        {notificationsOpen && (
          <aside className="absolute right-0 top-0 z-40 h-full w-[420px] border-l border-brand-800 bg-brand-950/95 p-6 shadow-panel backdrop-blur">
            <div className="mb-6 flex items-center justify-between">
              ...
              <button className="btn-secondary" onClick={toggleNotifications}>Close</button>
            </div>
                  ...
                  <div key={notification._id} className="panel p-4">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex gap-2">
                        {!notification.isRead && <button className="btn-secondary px-3 py-2" onClick={() => orgService.readNotification(notification._id).then(() => setNotifications((items) => items.map((item) => item._id === notification._id ? { ...item, isRead: true } : item)))}>Read</button>}
                        <button className="btn-secondary px-3 py-2" onClick={() => orgService.deleteNotification(notification._id).then(() => setNotifications((items) => items.filter((item) => item._id !== notification._id)))}>Delete</button>
                      </div>
                    </div>
                  </div>
                ))}
